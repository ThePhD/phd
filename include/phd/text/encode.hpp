...
namespace phd {
	...
	constexpr void text_encode_into(__Input&& __input, __Output&& __output, __Encoding&& __encoding, __State& __state, __ErrorHandler&& __error_handler) {
	...
	auto text_encode(__Input&& __input, __Encoding&& __encoding, __State& __state, __ErrorHandler&& __error_handler) {
		using __uEncoding = typename meta::remove_cv_ref<__Encoding>::type;
		using __code_unit = encoding_code_unit_t<__uEncoding>;
		...
		text_encode_into(std::forward<__Input>(__input), ranges::unbounded_view(ranges::back_inserter(__output)), std::forward<__Encoding>(__encoding), __state, std::forward<__ErrorHandler>(__error_handler));
		return __output;
	}

	template <typename __Input, typename __Encoding, typename __ErrorHandler>
	auto text_encode(__Input&& __input, __Encoding&& __encoding, __ErrorHandler&& __error_handler) {
		using __uEncoding = typename meta::remove_cv_ref<__Encoding>::type;
		using __state = encoding_state_t<__uEncoding>;

		__state __s{};
		return text_encode(std::forward<__Input>(__input), std::forward<__Encoding>(__encoding), __s, std::forward<__ErrorHandler>(__error_handler));
	}

	template <typename __Input, typename __Encoding>
	auto text_encode(__Input&& __input, __Encoding&& __encoding) {
		return text_encode(std::forward<__Input>(__input), std::forward<__Encoding>(__encoding), default_text_error_handler{});
	}
