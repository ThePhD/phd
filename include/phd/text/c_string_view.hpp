...
#endif // char8_t hacks
		constexpr inline u16c_string_view operator"" _cu16sv(const char16_t* str_, size_t len_) noexcept {
			return u16c_string_view(str_, len_);
		}
		constexpr inline u32c_string_view operator"" _cu32sv(const char32_t* str_, size_t len_) noexcept {
			return u32c_string_view(str_, len_);
		}
		constexpr inline wc_string_view operator"" _cwsv(const wchar_t* str_, size_t len_) noexcept {
