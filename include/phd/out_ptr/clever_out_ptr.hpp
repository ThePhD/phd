...
namespace phd {
	...
	struct clever_out_ptr_t : public out_ptr_detail::clever_out_ptr_impl<Smart, Pointer, Args, List> {
	private:
		using base_t = out_ptr_detail::clever_out_ptr_impl<Smart, Pointer, Args, List>;
	...
	auto clever_out_ptr(Smart& p, Args&&... args) noexcept {
		...
		return clever_out_ptr_t<Smart, Pointer, T, List>(p, std::forward_as_tuple(std::forward<Args>(args)...));
	}
	...
	auto clever_out_ptr(Smart& p, Args&&... args) noexcept {
		...
		return clever_out_ptr<Pointer>(p, std::forward<Args>(args)...);
	}
