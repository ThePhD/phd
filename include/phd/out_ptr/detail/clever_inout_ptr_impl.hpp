...
// this is not true of movelib,
// which stores a base deleter then m_p pointer type in a data class:
// just libc++'s implementation
...
namespace detail {
	...
	struct PHD_OUT_PTR_TRIVIAL_ABI inout_unique_fast : public voidpp_op<inout_unique_fast<Smart, T, D, Pointer>, Pointer> {
	public:
	...
	class PHD_OUT_PTR_TRIVIAL_ABI clever_inout_ptr_impl<std::unique_ptr<T, D>, Pointer, std::tuple<>, std::index_sequence<>,
	...
	template <typename T, typename D, typename Pointer>
	class PHD_OUT_PTR_TRIVIAL_ABI clever_inout_ptr_impl<boost::movelib::unique_ptr<T, D>, Pointer, std::tuple<>, std::index_sequence<>,
		typename std::enable_if<
