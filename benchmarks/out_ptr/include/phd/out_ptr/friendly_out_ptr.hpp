...
namespace phd { namespace out_ptr { namespace detail {
	...
	class friendly_out_ptr_impl<std::friendly_unique_ptr<T, D>, Pointer, std::tuple<>, std::index_sequence<>>
	: public voidpp_op<friendly_out_ptr_impl<std::friendly_unique_ptr<T, D>, Pointer, std::tuple<>, std::index_sequence<>>, Pointer> {
	public:
	...
	class friendly_out_ptr_t : public friendly_out_ptr_impl<Smart, Pointer, std::tuple<Args...>, std::make_index_sequence<std::tuple_size_v<std::tuple<Args...>>>> {
	private:
		using list_t = std::make_index_sequence<std::tuple_size_v<std::tuple<Args...>>>;
		using core_t = friendly_out_ptr_impl<Smart, Pointer, std::tuple<Args...>, list_t>;
