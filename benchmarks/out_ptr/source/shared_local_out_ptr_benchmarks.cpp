...
static void manual_shared_local_out_ptr(benchmark::State& state) {
	int64_t x = 0;
	for (auto _ : state) {
...
BENCHMARK(manual_shared_local_out_ptr)
	->ComputeStatistics("max", &compute_max)
	->ComputeStatistics("min", &compute_min)
	->ComputeStatistics("dispersion", &compute_index_of_dispersion);

static void simple_shared_local_out_ptr(benchmark::State& state) {
	int64_t x = 0;
	for (auto _ : state) {
		std::shared_ptr<ficapi::opaque> p(nullptr, ficapi::handle_no_alloc_deleter());
...
BENCHMARK(simple_shared_local_out_ptr)
	->ComputeStatistics("max", &compute_max)
	->ComputeStatistics("min", &compute_min)
	->ComputeStatistics("dispersion", &compute_index_of_dispersion);

static void clever_shared_local_out_ptr(benchmark::State& state) {
	int64_t x = 0;
	for (auto _ : state) {
		std::shared_ptr<ficapi::opaque> p(nullptr, ficapi::handle_no_alloc_deleter());
		ficapi_handle_no_alloc_create(phd::clever_out_ptr(p, ficapi::handle_no_alloc_deleter()));
		x += ficapi_handle_get_data(p.get());
	}
	int64_t expected = int64_t(state.iterations()) * ficapi_get_data();
	if (x != expected) {
...
BENCHMARK(clever_shared_local_out_ptr)
	->ComputeStatistics("max", &compute_max)
	->ComputeStatistics("min", &compute_min)
	->ComputeStatistics("dispersion", &compute_index_of_dispersion);
