...
TEST_CASE("out_ptr/basic", "out_ptr type works with smart pointers and C-style output APIs") {
	...
	SECTION("shared_ptr<void>") {
		...
		ficapi_create(phd::out_ptr::out_ptr(p, ficapi::deleter<>()), ficapi_type::ficapi_type_int);
		int* rawp = static_cast<int*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	}
	SECTION("shared_ptr<int>") {
		...
		ficapi_int_create(phd::out_ptr::out_ptr(p, ficapi_int_delete));
		int* rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	}
	...
	SECTION("shared_ptr<ficapi::opaque>, void out_ptr") {
		std::shared_ptr<ficapi::opaque> p(nullptr);
		ficapi_create(phd::out_ptr::out_ptr<void*>(p, ficapi_handle_delete), ficapi_type::ficapi_type_opaque);
		ficapi::opaque_handle rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(ficapi_handle_get_data(rawp) == ficapi_get_dynamic_data());
	}
...
TEST_CASE("out_ptr/stateful", "out_ptr type works with stateful deleters in smart pointers") {
	...
	SECTION("shared_ptr<void>, stateful_deleter") {
		...
		ficapi_create(phd::out_ptr::out_ptr(p, ficapi::stateful_deleter{ 0x12345678, ficapi_type::ficapi_type_int }), ficapi_type::ficapi_type_int);
		int* rawp = static_cast<int*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	...
	SECTION("shared_ptr<int>, stateful_int_deleter") {
		...
		ficapi_int_create(phd::out_ptr::out_ptr(p, ficapi::stateful_int_deleter{ 0x12345678 }));
		int* rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	...
	SECTION("shared_ptr<void>, stateful_deleter ref") {
		...
		ficapi_create(phd::out_ptr::out_ptr(p, std::ref(del)), ficapi_type::ficapi_type_int);
		int* rawp = static_cast<int*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	...
	SECTION("shared_ptr<int>, stateful_int_deleter ref") {
		...
		ficapi_int_create(phd::out_ptr::out_ptr(p, std::ref(del)));
		int* rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
...
TEST_CASE("out_ptr/simple-bases", "out_ptr will work with statically-castable base/derived relationships") {
	SECTION("base1 to derived cast") {
		std::unique_ptr<phd::out_ptr::test::base1> p(nullptr);
		int err = init_derived2(phd::out_ptr::out_ptr<phd::out_ptr::test::derived2*>(p), false);
		REQUIRE(err == 0);
		phd::out_ptr::test::derived2* rawp = static_cast<phd::out_ptr::test::derived2*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(phd::out_ptr::test::check_derived2(*rawp));
	}
	SECTION("base2 to derived cast") {
		std::unique_ptr<phd::out_ptr::test::base2> p(nullptr);
		int err = init_derived2(phd::out_ptr::out_ptr<phd::out_ptr::test::derived2*>(p), false);
		REQUIRE(err == 0);
		phd::out_ptr::test::derived2* rawp = static_cast<phd::out_ptr::test::derived2*>(p.get());
		REQUIRE(rawp != nullptr);
	...
	SECTION("base to derived1 to void cast") {
		std::unique_ptr<phd::out_ptr::test::base1> p(nullptr);
		int err = erased_init_derived1(phd::out_ptr::out_ptr<phd::out_ptr::test::derived1*>(p), false);
		REQUIRE(err == 0);
		phd::out_ptr::test::derived1* rawp = static_cast<phd::out_ptr::test::derived1*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(phd::out_ptr::test::check_derived1(*rawp));
	}
	SECTION("base to derived2 to void cast") {
		std::unique_ptr<phd::out_ptr::test::base2> p(nullptr);
		int err = erased_init_derived2(phd::out_ptr::out_ptr<phd::out_ptr::test::derived2*>(p), false);
		REQUIRE(err == 0);
		phd::out_ptr::test::derived2* rawp = static_cast<phd::out_ptr::test::derived2*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(phd::out_ptr::test::check_derived2(*rawp));
	...
	SECTION("DANGER: base2 to void** that is supposed to be derived2**") {
		std::unique_ptr<phd::out_ptr::test::base2> p(nullptr);
		int err = erased_init_derived2(phd::out_ptr::out_ptr<void*>(p), false);
		REQUIRE(err == 0);
		phd::out_ptr::test::derived2* rawp = static_cast<phd::out_ptr::test::derived2*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE_FALSE(phd::out_ptr::test::check_derived2(*rawp));
	}
}
...
TEST_CASE("out_ptr/roundtrip", "out_ptr should not allow round-tripping from void** to Pointer* and then void** if specific Pointer template arugment is used") {
	...
	SECTION("shared_ptr<void>") {
		std::shared_ptr<void> p(nullptr);
		ficapi_create(phd::out_ptr::out_ptr<ficapi::opaque_handle>(p, ficapi::deleter<ficapi_type::ficapi_type_opaque>()), ficapi_type::ficapi_type_opaque);
		ficapi::opaque_handle rawp = static_cast<ficapi::opaque_handle>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(ficapi_handle_get_data(rawp) == ficapi_get_dynamic_data());
	}
